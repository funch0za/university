...
(например, если угадано 15 изображений из 20, то точность равна 75%). 
Выведите получившуюся точность в консоль
'''
...
class GraphicsNetwork:
    ...
    def show_predictions(self, num_images=5):
        ...
        for i in range(num_images):
            ...
            print(f"Predict: {classes[predicted[i]]}\tCorrect:{classes[labels[i]]}")

            if classes[predicted[i]] == classes[labels[i]]:
                ok += 1

        print("Predict accuracy:", ok / num_images * 100)
...
net.show_predictions(num_images=10)
